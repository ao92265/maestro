...
/// and report the outcome. Tests inject a recorder; the app injects
/// [`system_runner`].
pub type GateCommandRunner =
    Arc<dyn Fn(&Path, &str, &[&str]) -> Result<GateCommandOutput, String> + Send + Sync>;
...
/// Which stage blocked the launch — the two failure classes the launcher
/// audits and surfaces distinctly (issue #90b).
...
/// The real runner: `std::process::Command`, output fully buffered. On
/// Windows everything goes through `cmd /C` — npm is a `.cmd` shim that
/// `CreateProcess` cannot spawn directly (the `ai_runner` claude-spawn
/// precedent) — and the console window is hidden as everywhere else.
pub fn system_runner() -> GateCommandRunner {
    Arc::new(|cwd, program, args| {
...
fn run_gate(
    ...
    if worktree.join("package.json").is_file() {
        ...
        match runner(worktree, "npm", &["install"]) {
    ...
    match runner(&cargo_dir, "cargo", &["test", "--workspace"]) {
        Ok(out) if out.success => {
