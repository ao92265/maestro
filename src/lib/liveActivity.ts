...
export function deriveLiveActivity(events: readonly ClaudeEvent[]): LiveActivity | null {
  let lastTool: LiveToolCall | null = null;
  let lastMessage: LiveMessage | null = null;

  for (let i = events.length - 1; i >= 0 && (!lastTool || !lastMessage); i--) {
    const event = events[i];
    if (!lastTool && event.event_type === "ToolUseStarted") {
      lastTool = {
  ...
  return { lastTool, lastMessage, updatedAt: stamps.sort()[stamps.length - 1] };
}
